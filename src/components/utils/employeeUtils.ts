...
import type { EmployeeProfile, EmployeeFormState } from '../../types/types';
...
export const normalizeCSVEmployeeData = (csvData: Record<string, unknown>): Partial<EmployeeFormState> => {
  ...
  const validateEmployeeType = (value: unknown): 'employee' | 'contractor' | 'transporter' | 'driver' | 'pending' => {
    const validTypes = ['employee', 'contractor', 'transporter', 'driver', 'pending'];
    const normalized = String(value || 'employee').trim().toLowerCase();
    return validTypes.includes(normalized) ? normalized as any : 'employee';
  };

  // Validate and normalize status
  const validateStatus = (value: unknown): 'active' | 'inactive' | 'terminated' | 'pending' => {
    const validStatuses = ['active', 'inactive', 'terminated', 'pending'];
    const normalized = String(value || 'active').trim().toLowerCase();
    return validStatuses.includes(normalized) ? normalized as any : 'active';
  };
